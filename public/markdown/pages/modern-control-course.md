...
平台质心处建立全局坐标系 $O$。论文模型把隔振平台视为刚体，虽然刚体本身有六个自由度，但在研究频段内只保留三个主要自由度：
...
论文参数中，平台质量为 $m=60\,\mathrm{kg}$，绕 $Y_O$ 轴转动惯量为 $J_{yy}=4.5\,\mathrm{kg\,m^2}$；竖向刚度 $k_s=4200\,\mathrm{N/m}$，水平刚度 $k_h=2060\,\mathrm{N/m}$；竖向阻尼 $d_s=100\,\mathrm{Ns/m}$，水平阻尼 $d_h=70\,\mathrm{Ns/m}$。这些参数用于说明模型量级；实际大作业应以给定仿真文件中的参数为准。
...
惯性传感器不是“理想测量器”。传感器内部有检验质量、柔性支撑、框架和读出单元。当平台加速时，检验质量相对框架运动并产生读数；同时，检验质量也会通过支撑结构向平台施加反作用力和反作用矩。论文中用
...
论文使用执行器和传感器的 Jacobian 矩阵把局部坐标投影到质心逻辑坐标。概念上可以理解为：

- 执行器侧：把三个局部执行器力组合成质心处的竖向广义力、转动广义力和水平广义力；
- 传感器侧：把局部传感器读数组合成质心处的竖向、转动和水平运动估计；
- 解耦后得到三输入三输出模型，输入和输出都对应同一参考点处的逻辑自由度。

这个过程不是把耦合“消灭”，而是把控制问题整理到更清楚的坐标系中。解耦后的传递函数矩阵中，对角项应当代表各自由度的主响应，非对角项应当明显减小。论文结果中仍能看到水平和转动方向之间存在残余耦合，原因是水平惯性传感器难以完全区分真实平动加速度和由转动、重力投影引起的等效测量。

### 动态特性与设计难点

论文给出的理论频响中，平台存在三个主要共振峰：竖向约 $1.8\,\mathrm{Hz}$，水平约 $1.3\,\mathrm{Hz}$，绕 $Y_O$ 轴转动约 $2.6\,\mathrm{Hz}$。这些共振频率落在主动隔振控制最关心的低频段内，因此控制器需要在低频提供足够扰动抑制能力。
...
这一部分就是大作业需要完成的任务。报告应围绕给定模型展开，不要求复现论文全部实验系统，但必须把模型、分析、控制器和评价标准说清楚。
...
论文中的增强方案还引入虚拟传感器融合。其思想是：低频使用真实惯性传感器信号以保留实际低频运动信息，高频使用由理论模型构造的虚拟传感器信号以避开未建模高频模态。互补滤波器在约 $10\,\mathrm{Hz}$ 处交叉，低通支路用于真实传感器，高通支路用于虚拟传感器；若 $20$ 到 $50\,\mathrm{Hz}$ 附近存在明显振荡模态，还可以在低通支路叠加陷波滤波器。这个结构不是本大作业必须完成的内容，但它解释了为什么高频测量噪声和未建模动态会限制控制器带宽。
